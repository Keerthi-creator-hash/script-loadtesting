...
const SPECIAL_TEACHER_USERNAME = "teacherXLLLL1"; // <-- main teacher with special data
...
const SPECIAL_ASSIGNMENTS = 100;
const SPECIAL_NOTES = 100;
const SPECIAL_MESSAGES = 100;

// 🕐 Utility Delay (10 seconds)
async function delay(ms) {
  return new Promise((res) => setTimeout(res, ms));
}
const GLOBAL_DELAY = 10000; // 10 seconds between API calls
...
async function safePost(url, data, label, token = null, retries = 3) {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      await delay(GLOBAL_DELAY); // ⏱ Wait 10 seconds before each call
...
async function createStudents(teachers) {
  ...
  async function createStudentBatch(teacher, count) {
    ...
    for (let i = 0; i < count; i++) {
      const uname = `studentXLLLL${studentId}${teacher.userName.replace(/[^a-zA-Z0-9]/g, "")}`;
      const sData = {
...
async function updateStudentBatchAssignments(students, teachers) {
  ...
  for (const s of students) {
    ...
    try {
      await delay(GLOBAL_DELAY); // ⏱ Wait 10 seconds before each update
      await axios.put(
